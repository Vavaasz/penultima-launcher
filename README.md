...
- The full map package must include both generated normal automap tiles under `minimap\Minimap_Color_*` and map-related 15.23 assets under `assets` (`catalog-content.json`, `map-*`, `staticdata-*`, `staticmapdata-*`, `minimap-*`, `satellite-*`, `subarea-*`). `full_minimap.asset_file_count = 0` means the package is incomplete for custom/Cyclopedia map data.
- Full-map install must not delete player-discovered `game\minimap\Minimap_Color_*` files that are absent from the package. It can still overwrite packaged color tiles and remove invalid generated `Minimap_WaypointCost_*` caches, because bad waypoint-cost files can break click movement.
